...
def ReturnArgues():
    ...
    parser.add_argument(
        "--ircpath", type=str, action="append", nargs="*", help="IRC coordinate file"
    )
    parser.add_argument(
        "--irct21", type=str, action="append", nargs="*", help="IRC coordinate file"
    )
    parser.add_argument(
        "--lt", type=str, action="append", nargs="*", help="LT coordinate file"
    )
    parser.add_argument(
        "--fragment",
        type=int,
        action="append",
        nargs="*",
        help="atom number for each fragment",
    )
    parser.add_argument(
        "--strain", type=float, action="append", nargs="*", help="print strain energy"
    )
    parser.add_argument(
        "--VDD", type=int, action="append", nargs="*", help="print VDD charges"
    )
    parser.add_argument(
        "--hirshfeld",
        type=str,
        action="append",
        nargs="*",
        help="print hirshfeld charges",
    )
    parser.add_argument(
        "--bondlength", type=foat, action="append", nargs="*", help="print bond length"
    )
    parser.add_argument(
        "--angle", type=float, action="append", nargs="*", help="print angle"
    )
    parser.add_argument(
        "--irrepOI",
        type=str,
        nargs="*",
        action="append",
        help="print OI energy for point group symmetry irrep",
    )
    parser.add_argument(
        "--population",
        type=str,
        nargs="*",
        action="append",
        help="print population for fragment orbital",
    )
    parser.add_argument(
        "--overlap",
        type=str,
        nargs="*",
        action="append",
        help="print overlap between two fragment orbitals",
    )
    parser.add_argument(
        "--orbitalenergy",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="*",
        action="append",
        help="a file containing adfinput parameters set",
    )
            ...
            if key == "overlap":
                for term in val:
                    if len(term) == 4:
                        inputValue.append(
                            (
                                {"type": term[1], "frag": term[0]},
                                {"type": term[3], "frag": term[2]},
                            )
                        )
                    else:
                        inputValue.append(
                ...
                inputKeys[key] = [
                    {"bondDef": [term[0], term[1]], "oriVal": term[2]} for term in val
                ]

            elif key == "angle":
                inputKeys[key] = [
                    {"angleDef": [term[0], term[1], term[2]], "oriVal": term[3]}
                    for term in val
                ]
                ...
                inputKeys[key] = [
                    "sett.input." + adfkey + '="' + keyval + '"'
                    for adfkey, keyval in adfinputList
                ]
                ...
                inputKeys[key] = [
                    "sett.input." + adfkey + '="' + keyval.strip("\n") + '"'
...
def readIRCPath(f, tag, offset):
    ...
    if (
        f.read(tag, "PathStatus").strip() == "DONE"
        or f.read(tag, "PathStatus").strip() == "EXEC"
    ):
...
def ReadIRCt21(fileName, fileName2=None):
    ...
    return [
        [
            [s, x, y, z]
            for s, x, y, z in zip(
                sAtoms, xyzBlock[0::3], xyzBlock[1::3], xyzBlock[2::3]
            )
        ]
        for xyzBlock in fwdIRC + cenIRC + bwdIRC
    ]
   ...
   """
    ...
    for coordBlock in ircStructures:
        if nAtoms != len(coordBlock):
            raise RuntimeError(
                "nAtoms in fragment definition does not match length if IRC coordinates\n"
            )
        # loop over IRC points
        ircList.append(
            dict([(fragTag, Molecule()) for fragTag in fragDefinition.keys()])
        )
        for fragTag in fragDefinition.keys():
            ...
            for iAtom in fragDefinition[fragTag]:
                ...
                ircList[-1][fragTag].add_atom(
                    Atom(
                        symbol=coordBlock[iAtom - 1][0],
                        coords=tuple(
                            [float(xyz) for xyz in coordBlock[iAtom - 1][1:4]]
                        ),
                    )
                )
   ...
   """
    ...
    for coordBlock, fragDefinition in zip(ircStructures, fragDefinitions):
        ...
        for i in range(len(coordBlock)):
            complexDict["complex"].add_atom(
                Atom(
                    symbol=coordBlock[int(i)][0],
...
def PrintTable(cellList, widthlist, bar):
    ...
    for i, entry in enumerate(cellList):
        print(
            "  " + str(entry).ljust(widthlist[i]) + "  "
        )  # TODO: consider number formatting here (if entry is not a string)
    print("")
    if bar:
        print(line)
...
def PyFragDriver(inputKeys, fragmentSettings, complexSettings):
    ...
    for key, val in inputKeys[
        "coordFile"
    ].items():  # TODO: rename coordFile to pathCoordFile
        if key == "irct21":
    ...
    successCases = []
    failStructures = []
    for ircIndex, ircFrags in enumerate(
        GetIRCFragmentList(ircStructures, inputKeys["fragment"])
    ):
        outputData = {}
        outputData["StrainTotal"] = 0
        complexMolecule = (
            Molecule()
        )  # each molecule is a subject of plams class Molecule()
        ircTag = "." + str(ircIndex + 1).zfill(5)
        # for fragTag in ircFrags.keys():
        for fragTag in list(ircFrags.keys()):
            success = True
            jobFrag = ADFJob(
                molecule=ircFrags[fragTag],
            ...
            if jobFrag.check():
                ...
                exec(
                    "complexSettings.input.Fragments."
                    + fragTag
                    + "="
                    + '"'
                    + jobFrag.results._kfpath()
                    + '"'
                )
                # get strain and total strain which is the energy difference between current and previous geometry.
                outputData[fragTag + "Strain"] = (
                    jobFrag.results.readkf("Energy", "Bond Energy")
                    - inputKeys["strain"][fragTag]
                )
        ...
        if success:
            ...
            jobComplex = ADFJob(
                ...
                name=complexMolecule.get_formula() + ircTag,
            )
            jobComplex.run()
            if jobComplex.check():
                ...
                pyfragResult = PyFragResult(
                    jobComplex.results, inputKeys
                )  # TODO: See __init__ of PyFragResult below
                # convert multiple value into a dict
                outputdata = pyfragResult.GetOutputData(
                    complexMolecule, outputData, inputKeys
                )
                ...
                valuesList = [
                    str(outputdata[i]) for i in headerList
                ]  # TODO: add formatting
                widthlist = [
                    max(len(str(valuesList[_])), len(str(headerList[_])))
                    for _ in range(len(valuesList))
                ]
    ...
    if len(resultsTable) == 0:
        raise RuntimeError(
            "Calculations for all points failed, please check your input settings"
        )
    return (
        resultsTable,
        str(complexMolecule.get_formula()),
        failStructures,
    )  # return this as result only (only construct it here but use it outside)


class PyFragResult:
    def __init__(
        self, complexResult, inputKeys
    ):  # __init__(self, complexJob, inputKeys)
    ...
    def GetFaIrrep(self):
        ...
        if len(self.irrepType) == 1:
            ...
            irreporbNum = self.irrepOrbNum
        faIrrepone = [
            [irrep for i in range(number)]
            for irrep, number in zip(self.irrepType, irreporbNum)
        ]
    ...
    def GetAtomNum(self, fragmentList, atoms):
        ...
        atomList = [
            atomNum
            for key in sorted(list(fragmentList.keys()))
            for atomNum in list(fragmentList[key])
        ]
    ...
    def GetOrbitalIndex(self, orbDescriptor):
        ...
        if orbDescriptor["type"] == "HOMO":
            index = max(
                xrange(len(self.orbEnergy)),
                key=lambda x: self.orbEnergy[x]
                if (self.orbFragment[x] == fragOrbnum) and self.orbOccupation[x] != 0
                else -1.0e100,
            )
            ...
            for i in range(len(self.orbEnergy)):
                if (
                    self.orbFragment[i] == fragOrbnum
                    and self.fragIrrep[i] == orbDescriptor["irrep"]
                    and self.fragOrb[i] == int(orbDescriptor["index"])
                ):
                    index = i
                    break
        return index
    ...
    def ReadOverlap(self, index_1, index_2):
        ...
        if self.GetFaIrrep([index_1]) == self.GetFaIrrep([index_2]):
            self.overlap_matrix = self.complexResult.readkf(
                self.GetFaIrrep([index_1]), "S-CoreSFO"
            )
    ...
    def ReadFragorbEnergy(self, index):
        return self.complexResult.readkf(
            "Ftyp " + str(self.orbFragment[index]) + self.fragIrrep[index], "eps"
        )[self.fragOrb[index] - 1]

    def ReadIrrepOI(self, irrep):
        irrepOI = [
            self.complexResult.readkf("Energy", "Orb.Int. " + irrep)
            for irrep in self.irrepType
        ]
    ...
    def ReadVDD(self, atomList):
        ...
        for atom in atomList:
            vddScf = self.complexResult.readkf("Properties", "AtomCharge_SCF Voronoi")[
                int(atom) - 1
            ]
            vddInit = self.complexResult.readkf(
    ...
    def ReadHirshfeld(self, fragment):
        valueHirshfeld = self.complexResult.readkf(
    ...
    def GetOutputData(self, complexMolecule, outputData, inputKeys):
        ...
        outputData["EnergyTotal"] = (
            Units.convert(self.Int, "hartree", "kcal/mol") + outputData["StrainTotal"]
        )
        ...
        for key, val in list(inputKeys.items()):
            ...
            if key == "overlap":
                outputData[key] = [
                    self.ReadOverlap(
                        self.GetOrbitalIndex(od1), self.GetOrbitalIndex(od2)
                    )
                    for od1, od2 in val
                ]
                ...
                outputData[key] = [
                    self.ReadPopulation(self.GetOrbitalIndex(od)) for od in val
                ]

            elif key == "orbitalenergy":
                outputData[key] = [
                    self.ReadFragorbEnergy(self.GetOrbitalIndex(od)) for od in val
                ]
                ...
                for od in val:
                    ...
                    for coorKey, coorVal in list(inputKeys["coordFile"].items()):
                        if coorKey == "ircpath":
                            value.append(
                                complexMolecule[atoms[0]].distance_to(
                                    complexMolecule[atoms[1]]
                                )
                                - od["oriVal"]
                            )
                            print(
                ...
                for od in val:
                    ...
                    value.append(
                        Units.convert(
                            (
                                complexMolecule[atoms[0]].angle(
                                    complexMolecule[atoms[1]], complexMolecule[atoms[2]]
                                )
                            ),
                            "rad",
                            "deg",
...
class PyFragJob:
    ...
    def run(self):
        tableValue, fileName, failStructures = PyFragDriver(
            self.inputArgues, self.fragmentSettings, self.complexSettings
        )
