...
from pyparsing import alphanums, Group, Literal, OneOrMore, Regex, Suppress, Word
from .parser import (
    floatNumber,
    integer,
    parse_file,
...
def parse_molecule(file_name):
    ...
    p1 = skipSupress(header1 | header2) + skipLine
    p2 = skipSupress("---\n") + skipLine
    parse_atoms = Group(Word(alphanums) + Suppress(floatNumber) + floatNumber * 3)
