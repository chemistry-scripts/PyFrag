...
def load_ticker(ticker):
    ...
    data = pd.read_csv(
        ...
        names=[
            ...
            "energy_change_",
            "e_TF",
            "constrained_gradient_max",
            "constrained_gradient_max_",
            "gm_TF",
            "constrained_gradient_rms",
            "constrained_gradient_rms_",
            "gr_TF",
            "cart_step_max",
            "cart_step_max_",
            "sm_TF",
    ...
    return pd.DataFrame(
        {
            ticker: data.energy,
            "E": data.energy_change,
            "E_": data.energy_change_,
            "e_TF": data.e_TF,
            "c_max": data.constrained_gradient_max,
            "c_max_": data.constrained_gradient_max_,
            "gm_TF": data.gm_TF,
            "c_rms": data.constrained_gradient_rms,
            "c_rms_": data.constrained_gradient_rms_,
            "gr_TF": data.gr_TF,
            "s_max": data.cart_step_max,
            "s_max_": data.cart_step_max_,
            "sm_TF": data.sm_TF,
            "s_rms": data.cart_step_rms,
            "s_rms_": data.cart_step_rms_,
            "sr_TF": data.sr_TF,
        }
    )
...
if pyfrag_sign == False:
    ...
    p1.line(
        PYFRAG["bondlength"], PYFRAG["energytotal"], color="black", legend="energytotal"
    )
    p1.line(PYFRAG["bondlength"], PYFRAG["inter"], color="green", legend="inter")
    p1.line(
        PYFRAG["bondlength"], PYFRAG["straintotal"], color="red", legend="straintotal"
    )
    ...
    layout = column(
        main_row, series, gridplot([[p1, p2]], plot_width=400, plot_height=400)
    )
