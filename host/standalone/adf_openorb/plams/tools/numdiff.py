...
class NumGradResults(Results):
    def get_gradient(self, atom, coord, func=None):
        ...
        energies = [
            extract(self.job.children[(atom, coord, i)].results) for i in s.steps
        ]
...
class NumGradJob(MultiJob):
    ...
    def __init__(
        self, molecule, atoms=None, npoints=3, step=0.01, unit="angstrom", **kwargs
    ):
        ...
        s.jobtype = (
            None  # who is going to do the actual work and calculate single points
        )
    ...
    def prerun(self):
        ...
        for atom in atomlist:
            ...
            for axis in co:
                for i in s.steps:
                    v = (
                        s.step * i if axis == "x" else 0,
                        s.step * i if axis == "y" else 0,
                        s.step * i if axis == "z" else 0,
                    )
                    newmol = s.molecule.copy()
                    newmol.atoms[at - 1].translate(v, s.unit)
                    newname = self.name + str(at) + axis + str(i)
                    self.children[(at, axis, i)] = s.jobtype(
                        name=newname, molecule=newmol, settings=self.settings.child
                    )
