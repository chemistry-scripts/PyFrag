...
class PyFragResult:
    def __init__(self, complexResult):
        ...
        self.orbEnergy = sum(
            [
                ConvertList(complexResult.read(self.irrepType[i], "eps_A"))
                for i in range(len(self.irrepType))
            ],
            [],
        ) + sum(
            [
                ConvertList(complexResult.read(self.irrepType[i], "eps_B"))
                for i in range(len(self.irrepType))
            ],
            [],
        )
        # occupation of each orbitals which is either 0 or 1
        self.orbOccupation = sum(
            [
                ConvertList(complexResult.read(self.irrepType[i], "froc_A"))
                for i in range(len(self.irrepType))
            ],
            [],
        ) + sum(
            [
                ConvertList(complexResult.read(self.irrepType[i], "froc_B"))
                for i in range(len(self.irrepType))
            ],
            [],
        )
        ...
        self.irrepOrblist = [
            num + 1
            for i in range(len(self.irrepOrbNumber))
            for num in range(self.irrepOrbNumber[i])
        ] + [
            num + 1
            for i in range(len(self.irrepOrbNumber))
            for num in range(self.irrepOrbNumber[i])
        ...
        # [a1, a1, b1, b1, b2, b2]
        self.irrepTypelist = sum(
            [
                [irrep for i in range(number)]
                for irrep, number in zip(self.irrepType, self.irrepOrbNumber)
            ],
            [],
        ) + sum(
            [
    ...
    def GetOrbitalIndex(self, orbDescriptor):
        ...
        if self.GetFrontIndex(orbDescriptor["type"])["holu"] == "HOMO":
            ...
            for i in range(len(self.orbEnergy)):
                if self.irrepTypelist[i] == orbDescriptor[
                    "irrep"
                ] and self.irrepOrblist[i] == int(orbDescriptor["index"]):
                    orbIndex = i
                    break
        return orbIndex
    ...
    def GetOutputData(self, outputData, inputKeys):
        ...
        for key, val in list(inputKeys.items()):
            ...
            if key == "orbitalenergy":
                for od in val:
                    if od["type"] != "INDEX":
                        outputData[od["type"]] = Units.convert(
                            self.orbEnergy[self.GetOrbitalIndex(od)], "hartree", "eV"
                        )
                    else:
                        outputData[
                            od["irrep"] + "_" + od["index"] + "_A"
                        ] = Units.convert(
                            self.orbEnergy[self.GetOrbitalIndex(od)], "hartree", "eV"
                        )
                        outputData[
                            od["irrep"] + "_" + od["index"] + "_B"
                        ] = Units.convert(
                            self.orbEnergy[self.GetOrbitalIndex(od) + self.totalOrb],
                            "hartree",
                            "eV",
                        )
        return outputData
