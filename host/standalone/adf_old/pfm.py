...
def read_pauli_elstat_t21(t21):
    au2kcal = 627.50954
    return au2kcal * (
        t21.read("Energy", "Pauli Total")[0] - t21.read("Energy", "Pauli Kinetic")[0]
    )
...
def remove_sub_irreps(irreps):
    ...
    for i in irreps:
        irrep = re.sub(r":.*", "", i)
        if not irrep in new_irreps:
            new_irreps.append(irrep)
    return new_irreps
    ...
    for orb in orbs:
        if len(orb) < 3 and re.search(r"HOMO|LUMO", orb[-1]):
            ...
            if irrep1 == None or irrep2 == None:
                sys.stderr.write(
                    "requested HOMO/LUMO for overlap value does not seem to exist\n"
                )
                ol_list.append(None)
                continue
            ...
            if irrep1 != irrep2:
                sys.stderr.write(
                    "requested HOMO/LUMO orbital overlap values --> orbs do not have same symmetry!\n"
                )
                ol_list.append(0)
                continue
            else:
        ...
        if not irrep in omat_dict:
            omat_dict[irrep] = t21.read(irrep, "S-CoreSFO")
...
def find_holu_t21_fa(t21, holu):
    ...
    for i in range(1, len(orbs)):
        if orbs[-i][1] == 0.0 and orbs[-i - 1][1] != 0.0:
            holu_orb = [
                orbs[-i - 1 + homo_index][-2],
                orbs[-i - 1 + homo_index][-1],
            ]  # Orbital irrep & nr in the fragment analysis calc.
...
def find_holu_t21(t21, frag_nr, holu, frag_nrs=False):
    ...
    frag_orbs = []
    for e, o, i, f, fn, onf, irf in zip(
        energies, occs, irreps_frags, frag_orb_nrs, fragment_nr, orb_nrs_fa, irreps_fa
    ):
        if re.search("HOMO", holu) and o == 0.0:
            continue
        if re.search("LUMO", holu) and o != 0.0:
            continue
        if fn == int(frag_nr):
            frag_orbs.append([e, o, i, f, fn, onf, irf])
    frag_orbs.sort()

    if re.search("HOMO", holu):
        if len(frag_orbs) - 1 + homo_index < 0:
            return None, None
        holu_orb = [
            frag_orbs[-1 + homo_index][-1],
            frag_orbs[-1 + homo_index][-2],
        ]  # Orbital irrep & nr in the fragment analysis calc.
        holu_orb_frag = [frag_orbs[-1 + homo_index][-5], frag_orbs[-1 + homo_index][-4]]

    if re.search("LUMO", holu):
        holu_orb = [
            frag_orbs[homo_index - 1][-1],
            frag_orbs[homo_index - 1][-2],
        ]  # Orbital irrep & nr in the fragment analysis calc.
...
def read_vdd_atoms_t21(t21, atom_list):
    ...
    for atom_nr in atom_list:
        try:
            ...
            vdd_init = t21.read("Properties", "AtomCharge_initial Voronoi")[
                int(atom_nr) - 1
            ]
...
def bondlength_xyz(xyz, atom1, atom2):
    ...
    bondlength = math.sqrt(
        (xyz[bond[0] - 1][0] - xyz[bond[1] - 1][0]) ** 2
...
def angle_xyz(xyz, atom1, atom2, atom3):
    ...
    vec1 = [
        xyz[angle[0] - 1][0] - xyz[angle[1] - 1][0],
        xyz[angle[0] - 1][1] - xyz[angle[1] - 1][1],
        xyz[angle[0] - 1][2] - xyz[angle[1] - 1][2],
    ]
    vec2 = [
        xyz[angle[2] - 1][0] - xyz[angle[1] - 1][0],
        xyz[angle[2] - 1][1] - xyz[angle[1] - 1][1],
        xyz[angle[2] - 1][2] - xyz[angle[1] - 1][2],
    ]
...
def projectangle_xyz(xyz, atom1, atom2, atom3, atom4, atom5):
    ...
    vec3 = [
        xyz[angle[4] - 1][0] - xyz[angle[3] - 1][0],
        xyz[angle[4] - 1][1] - xyz[angle[3] - 1][1],
        xyz[angle[4] - 1][2] - xyz[angle[3] - 1][2],
    ]
...
def dihedralangle_xyz(xyz, atom1, atom2, atom3, atom4):
    ...
    vec3 = [
        xyz[angle[1] - 1][0] - xyz[angle[2] - 1][0],
        xyz[angle[1] - 1][1] - xyz[angle[2] - 1][1],
        xyz[angle[1] - 1][2] - xyz[angle[2] - 1][2],
    ]
    vec4 = [
        xyz[angle[3] - 1][0] - xyz[angle[2] - 1][0],
        xyz[angle[3] - 1][1] - xyz[angle[2] - 1][1],
        xyz[angle[3] - 1][2] - xyz[angle[2] - 1][2],
    ]
    n_vec1 = vector_outerproduct(vec1, vec2)
...
def read_irc_xyz_out(file_name):
    ...
    summary = re.compile(
        r"Summary of the IRC.*?End of Path Summary", re.DOTALL
    ).findall(f)
    if summary == []:
        summary = re.compile(
            r"Summary of the LT.*?End of Path Summary", re.DOTALL
        ).findall(f)
    if summary == []:
        return
    else:
    ...
    for j in range(irc_points):
        ...
        for k in range(len(atoms)):
            xyz_matrix.append(
                [float(x_values[k][j]), float(y_values[k][j]), float(z_values[k][j])]
            )
...
def average_mw_distance(xyzmat, atoms):
    ...
    for i in range(1, len(xyzmat)):
        ...
        for j in range(len(xyz1)):
            ...
            diff += math.sqrt(
                (xyz1[j][0] * m - xyz2[j][0] * m) ** 2
                + (xyz1[j][1] * m - xyz2[j][1] * m) ** 2
                + (xyz1[j][2] * m - xyz2[j][2] * m) ** 2
            )
        dist.append(diff + dist[-1])
...
def subs_geomvar(bondlength_vars, bondlengths, matrix_list):
    for i in range(len(bondlengths[0])):
        for j in range(len(bondlengths)):
            if float(bondlengths[j][i]) < 0 and re.search(
                r"\+" + bondlength_vars[j], matrix_list[i]
            ):
                matrix_list[i] = re.sub(
                    r"\+" + bondlength_vars[j], str(bondlengths[j][i]), matrix_list[i]
                )
            else:
                matrix_list[i] = re.sub(
                    bondlength_vars[j], str(bondlengths[j][i]), matrix_list[i]
                )
