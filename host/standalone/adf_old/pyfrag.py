...
def get_print_values(t21, pp):
    ...
    if pp.IrrepOI.args is not None:
        pp.IrrepOI.cv = read_oi_for_irreps_t21(t21)
    if pp.PauliDecomp.args is not None:
        pp.PauliDecomp.cv = [read_pauli_kinetic_t21(t21), read_pauli_elstat_t21(t21)]
    if pp.VDD.args is not None:
        pp.VDD.cv = read_vdd_atoms_t21(t21, pp.VDD.args)
    if pp.Hirsfeld.args is not None:
        pp.Hirsfeld.cv = read_hirshfeld_t21(t21)
    if pp.DipoleMoment.args is not None:
        pp.DipoleMoment.cv = read_dipole_t21(t21)

    if pp.Pop.args is not None:
        pp.Pop.cv = read_populations_t21(t21, pp.Pop.args)
    if pp.OrbEn.args is not None:
        pp.OrbEn.cv = read_frag_orb_energies_t21(t21, pp.OrbEn.args)
    if pp.OrbEnGap.args is not None:
        pp.OrbEnGap.cv = read_frag_orb_energies_gap_t21(t21, pp.OrbEnGap.args)
    if pp.Overlap.args is not None:
        pp.Overlap.cv = read_overlaps_t21(t21, pp.Overlap.args)
...
def make_xyz_matrix(matrix, nr_of_atoms):
    ...
    for i in range(0, len(matrix) - nr_of_atoms, nr_of_atoms * 3):
        ...
        for j in range(0, nr_of_atoms * 3, 3):
            coor.append(
                [
                    matrix[i + j] / bohr2angs,
                    matrix[i + j + 1] / bohr2angs,
                    matrix[i + j + 2] / bohr2angs,
                ]
            )
...
def read_t21(file_name1, file_name2=False, irc=True):
    ...
    atomtype_index = f.read("Geometry", "fragment and atomtype index")[
        -nr_of_atoms:
    ]  # designates the atom types to t21-atomnumbers
    atom_order_index = f.read("Geometry", "atom order index")[
        -nr_of_atoms:
    ]  # gives nrs of atomtypes as in input
    atom_list = []
    ...
    if irc:
        ...
        xyz_matrix = list(
            f.read("IRC", "xyz")
        )  # This value is the TS geometry, not to be found in the IRC_forward/backward variable
...
def read_gaussian_xyz_out(main_dir, output_file):
    ...
    for i in loop:
        ...
        if re.compile(r"Input orientation:", re.IGNORECASE).search(gsscript):
            if re.compile(r"Input orientation:", re.IGNORECASE).search(gsscript):
                testcace = re.compile(
                    r"Input orientation:" + r".*?" + r"Distance matrix",
                ...
                spec = re.compile(
                    r"Distance matrix (angstroms):" + r".*", re.IGNORECASE
                ).sub("", spec)
                spec = re.compile(r"Input orientation:" + r".*", re.IGNORECASE).sub(
                    "", spec
                )
                newspec = []
...
def new_adf(
    adfscript, dir, name, name_t21, atoms_block, extra, fragments=False, title=""
):
    ...
    eor = re.sub(r"[<\s]*", "", eor)
    new_adfscript = re.compile(r"ATOMS.*?END", re.DOTALL | re.IGNORECASE).sub(
        "", adfscript
    )
    new_adfscript = re.sub(
        r"<<.*",
        "<<eor> " + shelljoin(dir, name + ".out") + "\n\nATOMS\nEND",
        new_adfscript,
    )
    new_adfscript = re.sub(r"[^<a-zA-Z0-9]" + eor, "\neor\n", new_adfscript)

    if re.compile(r"TITLE.*", re.IGNORECASE).search(new_adfscript):
        new_adfscript = re.compile(r"TITLE.*", re.IGNORECASE).sub(
            "TITLE " + title, new_adfscript + "\n", 1
        )
    else:
        new_adfscript = re.compile(r"ATOMS", re.IGNORECASE).sub(
            "TITLE " + title + "\n\nATOMS", new_adfscript, 1
        )

    if re.search("cp\s*TAPE21.*", new_adfscript):
        new_adfscript = re.compile(r"cp\s*TAPE21.*", re.IGNORECASE).sub(
            "cp TAPE21 " + name_t21 + "\ncp logfile " + dir + "\n", new_adfscript
        )
    else:
        new_adfscript = re.sub(
    ...
    if fragments:
        if re.compile(r"FRAGMENTS.*?END", re.DOTALL | re.IGNORECASE).search(
            new_adfscript
        ):
            new_adfscript = re.compile(
                r"FRAGMENTS.*?END", re.DOTALL | re.IGNORECASE
            ).sub(fragments, new_adfscript)
        else:
            new_adfscript = re.compile(r"end\s*input", re.IGNORECASE).sub(
                fragments + "\nEND INPUT\n", new_adfscript
            )
...
def make_gnuplot_script(data_file, main_dir, pp, filename):
    ...
    skip_columns = [
        ...
        int(steric),
        int(oi),
    ]  # making list for the columns not automatically processed in the end of the script
    ...
    if pp.StrainFrag1.args is not None and pp.StrainFrag2.args is not None:
        ...
        skip_columns.extend(
            [int(strain_tot), int(energy_tot), int(strain_frag1), int(strain_frag2)]
        )

    decomp = [x + ":" + inten, x + ":" + pauli, x + ":" + elstat, x + ":" + oi]
    decomp_steric = [x + ":" + inten, x + ":" + steric, x + ":" + oi]

    gnuscript += make_graph(
        shelljoin(main_dir, data_file),
        "Decomposition Interaction Energy",
        [
            "{/Symbol D}E_{int}",
            "{/Symbol D}E_{Pauli}",
            "{/Symbol D}V_{Dlstat}",
    ...
    gnuscript += make_graph(
        shelljoin(main_dir, data_file),
        "Decomposition Interaction Energy",
        ["{/Symbol D}E_{int}", "{/Symbol D}E_{Steric}", "E_{oi}"],
        decomp_steric,
        xlabel=xlabel,
    )
    ...
    if pp.StrainFrag1.args is not None and pp.StrainFrag2.args is not None:
        gnuscript += make_graph(
            shelljoin(main_dir, data_file),
            "Strain and Interaction energy",
            ["{/Symbol D}E_{Total}", "{/Symbol D}E_{Strain}", "{/Symbol D}E_{Int}"],
            strain_int,
            xlabel=xlabel,
        )
        gnuscript += make_graph(
            shelljoin(main_dir, data_file),
            "Decomposition Strain",
            [
                "{/Symbol D}E_{Strain} (Total)",
                "{/Symbol D}E_{Strain} (Frag1)",
                "{/Symbol D}E_{Strain} (Frag2)",
            ],
            strain_decomp,
            xlabel=xlabel,
        )
...
def get_input_block(specs, match_string, match_string2, default=""):
    ...
    if re.compile(match_string, re.IGNORECASE).search(specs):
        if match_string2 and re.compile(match_string, re.IGNORECASE).search(specs):
            spec = re.compile(
                match_string + r".*?" + match_string2, re.IGNORECASE | re.DOTALL
            ).findall(specs)[0]
...
class Parameters:
    ...
    def add(
        ...
        name,
        specs="Default",
        match_string="Default",
        default=None,
        dim=0,
        print_names=False,
        unit="",
    ):
        self.dict[name] = InputPrm(
            name, specs, match_string, default, dim, print_names, unit
        )
...
specs = re.compile(r"PYFRAG\s*INPUT.*END\s*PYFRAG\s*INPUT", re.DOTALL).findall(
    adfscript
)
if not specs:
    specs = re.compile(r"INPUT_?\s*SPECS.*END INPUT_SPECS", re.DOTALL).findall(
        adfscript
    )
if specs != []:
    specs = specs[0]
...
adfscript = re.compile(
    r".*END\s*INPUT_?\s*SPECS|.*END\s*PYFRAG\s*INPUT", re.DOTALL
).sub("", adfscript, 1)
...
frag_dir = os.path.basename(
    get_input_line(specs, r"fragments?.*dir.(.*)", default="fragmentfiles")
)
data_file = os.path.basename(
    get_input_line(specs, r"data.*file.(.*)", default="fragment_energies.txt")
)
...
try:
    ...
    sys.stderr.write(
        "Temporary TAPE21 dir is already present:" + tape21_temp_dir + "\n"
    )
...
if lt == False and sp == False and irc == False:
    sys.stderr.write(
        "The type of pyfrag calculation (IRC/LT/SP) has not been specified.\n"
    )
...
pp.Filename.pwidth = (
    len(pp.Filename.args) + 7
)  # '_' and four numbers are added after the filenname, increase column width (plus 2 default extra)
pp.add("Point", default=[], unit="Nr.")

pp.add("Bondlength", specs, r"print.*bond\s*=?\s*(\d\s*\d.*)", dim=2, unit="Angstrom")
pp.add(
    "Angle", specs, r"print\s*_?angle\s*=?\s*(\d\s*\d\s*\d.*)", dim=2, unit="Degrees"
)
pp.add(
    "PlaneAngle",
    specs,
    r"print.*plane.*angle\s*=?\s*(\d\s*\d\s*\d\s*\d\s*\d.*)",
    dim=2,
    unit="Degrees",
)
...
pp.add(
    "DihedralAngle",
    specs,
    r"print\s*_?dihedralangle\s*=?\s*(\d\s*\d\s*\d\s*\d.*)",
    dim=2,
    unit="Degrees",
)
pp.add(
    "ProjectAngle",
    specs,
    r"print\s*_?projectangle\s*=?\s*(\d\s*\d\s*\d\s*\d\s*\d.*)",
    dim=2,
    unit="Degrees",
)
...
if sp:
    ...
    if pp.GeomVar.args is None:
        pp.add("GeomVar", specs, r"Bondlengths(.*)", dim=2, unit="Variable")
...
pp.add(
    "PauliDecomp",
    specs,
    r"print\s*\pauli\s*(decomp).*",
    print_names=["Pauli_Kinetic", "Pauli_Elstat"],
    unit="kcal/mol",
)
...
pp.add(
    ...
    specs,
    r"print\s*(hirshfeld)",
    print_names=["HirsfeldFrag1", "HirsfeldFrag2"],
    unit="a.u.",
)
...
if sp:
    ...
    for i in range(len(bondlengths)):
        try:
            ...
            if re.compile(r"step", re.IGNORECASE).search(
                bondlengths[i][0]
            ) or re.compile(r"step", re.IGNORECASE).search(bondlengths[i][1]):
                if re.compile(r"step", re.IGNORECASE).search(bondlengths[i][0]):
                    bondlengths[i][0] = "steps"
                if re.compile(r"step", re.IGNORECASE).search(bondlengths[i][1]):
                    bondlengths[i][1] = "steps"
                ...
                temp_bondlengths = []
                try:
                    bondlengths[i][0], bondlengths[i][1], bondlengths[i][2] = (
                        float(bondlengths[i][0]),
                        float(bondlengths[i][1]),
                        int(bondlengths[i][2]),
                    )
                except:
                    sys.stderr.write(
                        "Input for bondlength steps in error, please check your input file\n"
                    )
                    sys.stderr.write(
                        str(bondlengths[i][0])
                        + "  "
                ...
                if bondlengths[i][2] == 1:
                    ...
                    step = (bondlengths[i][1] - bondlengths[i][0]) / (
                        bondlengths[i][2] - 1
                    )
    ...
    if re.compile(r"give\s*tape21\s*frag1\s*=?.*", re.IGNORECASE).search(specs):
        name_frag1_t21_fa = shelljoin(
            main_dir, get_input_line(specs, r"give\s*tape21\s*frag1\s*=?(.*)")
        )
        ...
        for var in bondlength_vars:
            if re.search(var, frag1):
                ...
                matrix_list_frag1 = subs_geomvar(
                    bondlength_vars, bondlengths, matrix_list_frag1
                )
                break
    ...
    if re.compile(r"give\s*tape21\s*frag2\s*=?.*", re.IGNORECASE).search(specs):
        name_frag2_t21_fa = shelljoin(
            main_dir, get_input_line(specs, r"give\s*tape21\s*frag2\s*=?(.*)")
        )
        ...
        for var in bondlength_vars:
            if re.search(var, frag2):
                ...
                matrix_list_frag2 = subs_geomvar(
                    bondlength_vars, bondlengths, matrix_list_frag2
                )
                break
...
if lt or irc:
    ...
    if re.search(r".t21$", output_file):
        if irc and not output_file_2:
            atom_list, matrix = read_t21(os.path.join(main_dir, output_file))
        if irc and output_file_2:
            atom_list, matrix = read_t21(
                os.path.join(main_dir, output_file),
                os.path.join(main_dir, output_file_2),
            )
        if lt:
            atom_list, matrix = read_t21(os.path.join(main_dir, output_file), irc=False)
    ...
    # gaussian irc output file
        ...
        atom_list, matrix, energies = read_irc_xyz_out(
            os.path.join(main_dir, output_file)
        )
        if output_file_2:
            atom_list, matrix2, energies2 = read_irc_xyz_out(
                os.path.join(main_dir, output_file_2)
            )
            matrix.reverse()
            matrix.extend(matrix2)
    ...
    for xyz in matrix:
        ...
        for j in range(len(xyz)):
            if not j + 1 in LT_frag1 and not j + 1 in LT_frag2:
                continue
...
if pp.Bondlength.args is not None:
    for i in range(len(pp.Bondlength.args)):
        ...
        for xyz in matrix:
            bondlengths.append(
                bondlength_xyz(xyz, pp.Bondlength.args[i][0], pp.Bondlength.args[i][1])
                - diffb
            )
...
if pp.Angle.args is not None:
    for i in range(len(pp.Angle.args)):
        ...
        for xyz in matrix:
            angles.append(
                angle_xyz(
                    xyz, pp.Angle.args[i][0], pp.Angle.args[i][1], pp.Angle.args[i][2]
                )
                - diffa
            )
...
if pp.ProjectAngle.args is not None:
    for i in range(len(pp.ProjectAngle.args)):
        ...
        if len(pp.ProjectAngle.args[i]) > 5:
            ...
            diffa = 0
        for xyz in matrix:
            projectangles.append(
                projectangle_xyz(
                    xyz,
                    pp.ProjectAngle.args[i][0],
                    pp.ProjectAngle.args[i][1],
                    pp.ProjectAngle.args[i][2],
                    pp.ProjectAngle.args[i][3],
                    pp.ProjectAngle.args[i][4],
                )
                - diffa
            )
...
if pp.DihedralAngle.args is not None:
    for i in range(len(pp.DihedralAngle.args)):
        ...
        if len(pp.DihedralAngle.args[i]) > 4:
            ...
            diffa = 0
        for xyz in matrix:
            dihedralangles.append(
                dihedralangle_xyz(
                    xyz,
                    pp.DihedralAngle.args[i][0],
                    pp.DihedralAngle.args[i][1],
                    pp.DihedralAngle.args[i][2],
                    pp.DihedralAngle.args[i][3],
                )
                - diffa
            )
    ...
    for i in range(len(pp.PlaneAngle.args)):
        ...
        for xyz in matrix:
            plane_angles.append(
                plane_angle_xyz(
                    xyz,
                    pp.PlaneAngle.args[i][0],
                    pp.PlaneAngle.args[i][1],
                    pp.PlaneAngle.args[i][2],
                    pp.PlaneAngle.args[i][3],
                    pp.PlaneAngle.args[i][4],
                )
                - diffa
            )
...
if calc_points == []:
    for i in range(starting_point, number_of_loops):
        calc_points.append(i)
...
for i in calc_points:
    ...
    fragment_block = (
        "FRAGMENTS \nfrag1  "
        + new_name_frag1_t21_fa
        + "\nfrag2  "
        + new_name_frag2_t21_fa
        + "\nEND\n"
    )
    ...
    new_adf(
        ...
        extra_fa,
        fragment_block,
        title="fragment_analysis",
    )
    if matrix_list_frag1:
        os.remove(new_name_frag1_t21_fa)
    if matrix_list_frag2:
        os.remove(new_name_frag2_t21_fa)
    ...
    if densf:
        ...
        for orb in densf:
            if orb[0] == "frag1":
                frag_orb = find_holu_t21(
                    current_fa_t21,
                    get_frag_nr(current_fa_t21, orb[0]),
                    orb[1],
                    frag_nrs=True,
                )
                do_densf(
                    new_name_frag1_t21,
                    frag_orb,
                    shelljoin(frag_dir, new_name_frag1),
                    orb[1],
                )
            if orb[0] == "frag2":
                frag_orb = find_holu_t21(
                ...
                do_densf(
                    new_name_frag2_t21,
                    frag_orb,
                    shelljoin(frag_dir, new_name_frag2),
                    orb[1],
                )
            if orb[0] == "fa":
                fa_orb = find_holu_t21_fa(current_fa_t21, orb[1])
                do_densf(
                    new_name_fa_t21, fa_orb, shelljoin(frag_dir, new_name_fa), orb[1]
                )
...
if save_tape21s:
    ...
    try:
        ...
        sys.stderr.write(
            "Dir for saving TAPE21s already present :" + tape21_save_dir + "\n"
        )
    for file in os.listdir(tape21_temp_dir):
        shutil.copyfile(
            os.path.join(tape21_temp_dir, file), os.path.join(tape21_save_dir, file)
        )
...
if print_relative_energies is not None:
    ...
    for i in range(2, columns - 1):
        ...
        if column_keys[i + 1] != "GeomVar":
            data_cols[-1] = [
                nr - data_cols[-1][int(print_relative_energies)] for nr in data_cols[-1]
            ]
    ...
    if gnuplot is not None:
        make_gnuplot_script(
            ...
            main_dir,
            pp,
            sys.argv[1] + "_relative_pyfrag_gnuscript",
        )
