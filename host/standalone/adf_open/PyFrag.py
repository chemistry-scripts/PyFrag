...
from PyFragModules import PyFragDriver, WriteTable, WriteFailFiles, WriteDefaultTable
...
parser.add_argument(
    "--straintotal", type=float, action="append", nargs="*", help="print strain energy"
)
parser.add_argument(
...
parser.add_argument(
    ...
    action="append",
    nargs="*",
    help="restart directory of file, i.e. plams.xyzuvw",
)
parser.add_argument(
    "--name",
    type=str,
    action="append",
    nargs="*",
    help="provide name for the file generated by plams",
)
...
for key, val in vars(parser.parse_args()).items():
    if val != None:
        ...
        #    inputKeys[key] = adfComplex + adfFrag1 + adfFrag2
            ...
            adfGeneral = [
                "settings.input." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfFrag1 = [
                "settings_Frag1.input." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfFrag2 = [
                "settings_Frag2.input." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfComplex = [
                "settings_Fa.input." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfFrag1_open = [
                "settings_Frag1_open.input." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfFrag2_open = [
                "settings_Frag2_open.input." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfComplex_open = [
                "settings_Fa_open.input." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            inputKeys[key] = (
                adfGeneral
...
settings_Frag1_open = Settings()
settings_Frag2_open = Settings()
settings_Fa_open = Settings()
...
for key, val in list(inputKeys.items()):
    if key == "complex_EXTRA":
        for option in inputKeys["complex_EXTRA"]:
            exec(option)
for key, val in list(inputKeys.items()):
    if key == "fragment1_open_EXTRA":
        for option in inputKeys["fragment1_open_EXTRA"]:
            exec(option)
for key, val in list(inputKeys.items()):
    if key == "fragment2_open_EXTRA":
        for option in inputKeys["fragment2_open_EXTRA"]:
            exec(option)
for key, val in list(inputKeys.items()):
    if key == "complex_open_EXTRA":
        for option in inputKeys["complex_open_EXTRA"]:
            exec(option)

tableValues, fileName, failStructures = PyFragDriver(
    inputKeys,
    settings_Frag1,
    settings_Frag2,
    settings_Fa,
...
WriteTable(tableValues, fileName)
WriteDefaultTable(tableValues, fileName)
