...
def GetIRCFragmentList(ircStructures, fragDefinition):
   ...
   """
    ...
    for coordBlock in ircStructures:
        ...
        ircList.append(
            dict([(fragTag, Molecule()) for fragTag in list(fragDefinition.keys())])
        )
...
def PyFragDriver(
    ...
    for ircIndex, ircFrags in enumerate(
        ...
        if True:
            ...
            outputLine = pyfragResult.GetOutputData(
        ...
        for fragTag_open in sorted(list(ircFrags_open.keys())):
            ...
            jobFrag = ADFJob(
                molecule=ircFrags_open[fragTag_open],
                settings=fragmentSettings,
                name=fragTag_open + "open" + ircTag,
            )
            jobFrag.run()

            # get strain and total strain which is the energy difference between current and previous geometry.
            outputData_open[fragTag_open + "Strain"] = (
                Units.convert(
                    jobFrag.results.readkf("Energy", "Bond Energy"),
                    "hartree",
                    "kcal/mol",
                )
                - inputKeys["strain"][fragTag_open]
            )
        ...
        jobComplex = ADFJob(
            molecule=complexMolecule_open,
            settings=complexSettings_open,
            name=complexMolecule_open.get_formula() + "open" + ircTag,
        )
        jobComplex.run()
        if True:
            # if jobComplex.check():
            ...
            outputData_open["EnergyTotal"] = (
                Units.convert(
                    jobComplex.results.readkf("Energy", "Bond Energy"),
                    "hartree",
                    "kcal/mol",
                )
                - inputKeys["strain"]["frag1"]
                - inputKeys["strain"]["frag2"]
            )
            outputData_open["int"] = (
                outputData_open["EnergyTotal"] - outputData_open["straintotal"]
            )
            outputData_open["StrainTotal"] = (
                outputData_open["EnergyTotal"] - outputLine["Int"]
            )
            # convert multiple value into a dict
...
class PyFragResult:
    ...
    def ReadIrrepOI(self, irrep):
        irrepOI = [
            self.complexResult.readkf("Energy", "Orb.Int. " + irreps)
            for irreps in self.irrepType
        ]
        fitCoefficient = self.OI / sum(irrepOI)
        return fitCoefficient * Units.convert(
