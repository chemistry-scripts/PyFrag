...
def read_energy_t21(t21, unit="kcal/mol"):
    ...
    gout = input.read()
    input.close()
    energyline = re.compile(
        r"SCF Done:" + r".*?" + r"A.U. after", re.IGNORECASE | re.DOTALL
    ).findall(gout)
...
def new_adf(
    ...
    new_adfscript = re.sub(
        r"<<.*", "<<eor> " + shelljoin(dir, name + ".out"), new_adfscript
    )
...
for i in calc_points:
    ...
    if pp.StrainFrag1.args is not None:
        pp.StrainFrag1.cv = read_energy_t21(new_name_frag1_t21) - float(
            pp.StrainFrag1.args
        )
    else:
    ...
    if pp.StrainFrag2.args is not None:
        pp.StrainFrag2.cv = read_energy_t21(new_name_frag2_t21) - float(
            pp.StrainFrag2.args
        )
    else:
    ...
    if pp.StrainFrag1.args is not None and pp.StrainFrag2.args is not None:
        ...
        pp.EnergyTotal.cv = (
            read_energy_t21(new_name_fa_t21)
            - float(pp.StrainFrag1.args)
            - float(pp.StrainFrag2.args)
        )
