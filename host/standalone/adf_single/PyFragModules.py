   ...
   """
    ...
    for coordBlock in ircStructures:
        ...
        for iAtom in coordBlock:
            ...
            ircList[-1].add_atom(
                Atom(symbol=iAtom[0], coords=tuple([float(xyz) for xyz in iAtom[1:4]]))
            )
...
def PyFragDriver(inputKeys, complexSettings):
    ...
    for ircIndex, ircFrags in enumerate(GetIRCFragmentList(ircStructures)):
        ...
        jobComplex = ADFJob(
            ...
            name=ircFrags.get_formula() + ircTag,
        )
        jobComplex.run()
        if jobComplex.check():
            ...
            # collect all data that need to be printed
            outputData_open["EnergyTotal"] = Units.convert(
            ...
            if key == "VDD":
                ...
                for od in val:
                    ...
                    for coorKey, coorVal in list(inputKeys["coordFile"].items()):
                        if coorKey == "ircpath":
                            value.append(
                                complexMolecule[int(atoms[0])].distance_to(
                                    complexMolecule[int(atoms[1])]
                                )
                                - od["oriVal"]
                            )
                ...
                for od in val:
                    ...
                    value.append(
                        Units.convert(
                            (
                                complexMolecule[int(atoms[0])].angle(
                                    complexMolecule[int(atoms[1])],
                                    complexMolecule[int(atoms[2])],
                                )
                            ),
                            "rad",
                            "deg",
