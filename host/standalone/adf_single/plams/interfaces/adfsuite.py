...
class SCMResults(Results):
    ...
    def readkf(self, section, variable):
        ...
        raise FileError(
            "KFFile of %s not present in %s" % (self.job.name, self.job.path)
        )
    ...
    def get_molecule(self, section, variable, unit="bohr", internal=False, n=1):
        ...
        if self.job.molecule:
            ...
            return m
        raise PlamsError(
            "get_molecule() failed. Corresponding job has no molecule associated"
        )
...
class SCMJob(SingleJob):
    ...
    def check(self):
        ...
        try:
            ret = (
                self.results.readkf("General", "termination status").strip(" ")
                == "NORMAL TERMINATION"
            )
        except:
            return False
    ...
    def _atom_symbol(atom):
        ...
        smb = (
            atom.symbol if atom.atnum > 0 else ""
        )  # Dummy atom should have '' instead of 'Xx'
...
class ADFJob(SCMJob):
    ...
    def _parsemol(self):
        for i, atom in enumerate(self.molecule):
            ...
            self.settings.input.atoms["_" + str(i + 1)] = ("%5i" % (i + 1)) + atom.str(
                symbol=smb, suffix=suffix
            )
    ...
    def _parsemol(self):
        ...
        if self.molecule.lattice:
            for i, vec in enumerate(self.molecule.lattice):
                self.settings.input.lattice["_" + str(i + 1)] = (
                    "%16.10f %16.10f %16.10f" % vec
                )
...
class DFTBJob(SCMJob):
    ...
    def _parsemol(self):
        ...
        if self.molecule.lattice:
            for i, vec in enumerate(self.molecule.lattice):
                s[s.find_case("system")]["lattice"]["_" + str(i + 1)] = (
                    "%16.10f %16.10f %16.10f" % vec
                )
...
class FCFResults(SCMResults):
    ...
    def get_molecule(self, *args, **kwargs):
        raise PlamsError(
            "FCFResults do not support get_molecule() method. You can get molecules from job1 or job2"
        )
...
class DensfResults(SCMResults):
    ...
    def get_molecule(self, *args, **kwargs):
        raise PlamsError(
            "DensfResults do not support get_molecule() method. You can get molecule from inputjob"
        )
