...
parser.add_argument(
    "--r1path", type=str, action="append", nargs="*", help="IRC coordinate file"
)
parser.add_argument(
    "--r2path", type=str, action="append", nargs="*", help="IRC coordinate file"
)
parser.add_argument(
    "--rcpath", type=str, action="append", nargs="*", help="IRC coordinate file"
)
parser.add_argument(
    "--ppath", type=str, action="append", nargs="*", help="IRC coordinate file"
)
parser.add_argument(
    "--tspath", type=str, action="append", nargs="*", help="IRC coordinate file"
)
parser.add_argument(
...
parser.add_argument(
    "--hirshfeld", type=str, action="append", nargs="*", help="print hirshfeld charges"
)
parser.add_argument(
    "--bondlength", type=float, action="append", nargs="*", help="print bond length"
)
parser.add_argument(
...
parser.add_argument(
    "--orbitalenergy", type=str, nargs="*", action="append", help="print orbital energy"
)
...
for key, val in vars(parser.parse_args()).items():
    if val != None:
        ...
        if key == "overlap":
            ...
            for term in val:
                if len(term) == 2:
                    ...
                    inputValue.append(
                        ({"bondDef": [term[0], term[1]], "oriVal": term[2]})
                    )
            inputKeys[key] = inputValue
            ...
            for term in val:
                if len(term) == 3:
                    inputValue.append(
                        ({"angleDef": [term[0], term[1], term[2]], "oriVal": 0})
                    )
                else:
                    inputValue.append(
        ...
        #    inputKeys[key] = ['settings.specific.adf.'+adfkey+'="'+keyval+'"' for adfkey, keyval in adfinputList]
            ...
            adfGeneral = [
                "settings.specific.adf." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfR1 = [
                "settings_R1.specific.adf." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfR2 = [
                "settings_R2.specific.adf." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfRC = [
                "settings_RC.specific.adf." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfTS = [
                "settings_TS.specific.adf." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfP = [
                "settings_P.specific.adf." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfIRC = [
                "settings_IRC.specific.adf." + adfkey + '="' + keyval.strip("\n") + '"'
                for adfkey, keyval in adfinputLine
            ]
            adfFrag1 = [
                "settings_Frag1.specific.fragment1."
                + adfkey
                + '="'
                + keyval.strip("\n")
                + '"'
            ...
            adfFrag2 = [
                "settings_Frag2.specific.fragment2."
                + adfkey
                + '="'
                + keyval.strip("\n")
                + '"'
...
for key, val in list(inputKeys.items()):
    if key == "tspath":
        ...
        if "strain" in realargue.keys():
            pyfrag = pyfrag(
                templates.frag1.overlay(settings_Frag1),
                settings_2=templates.frag2.overlay(settings_Frag2),
                settings_3=templates.fa.overlay(settings_Fa),
                inputArgues=irc.kf.path,
                others=vars(parser.parse_args()),
                job_name="pyfrag",
            )
        else:
            pyfrag = pyfrag(
                ...
                settings_3=templates.fa.overlay(settings_Fa),
                inputArgues=irc.kf.path,
                inputArgues1=r1.kf.path,
                inputArgues2=r2.kf.path,
                others=vars(parser.parse_args()),
                job_name="pyfrag",
            )
